...
void ServerRouter::_addStatusCodePage(t_connection * connection, std::string & contentTypeAndLengthAndData)
{
	...
	if (fileOpen == NULL || S_ISDIR(buf.st_mode))
	{
		...
		contentTypeAndLengthAndData += "Content-Type: " + contType + "; charset=utf-8" + DELIMETER + "Content-Length: " + std::to_string(newStatusCodePage.size()) + DDELIMETER + connection->responseData.fileToSendInBinary + DDELIMETER;
	}
